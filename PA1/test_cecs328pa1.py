import unittest
from cecs328pa1 import asymptotic_analysis # type: ignore
...
class TestAsymptoticAnalysis(unittest.TestCase):
    ...
    def test_proffessor_test_cases(self):
        ...
        self.assertTrue(asymptotic_analysis("2*n**2 + n*log(n) + 5", "n**2", "Θ"))  # True
        self.assertTrue(asymptotic_analysis("n**2 + n + 5", "n*(log(n))**2", "Ω"))  # True
        self.assertFalse(asymptotic_analysis("log(n)**2", "lg(n**3)" ,"O"))
        self.assertFalse(asymptotic_analysis("log(n)^2", "lg(n^3)" ,"O"))
